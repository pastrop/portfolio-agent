...
`--test` takes precedence — if combined with `--model` / `--iterations` / `--no-refine` / `--no-advisor` / `--no-prices` / `--capital`, the test-mode defaults win. The one exception is `--max-loss`, which is orthogonal and applies even under `--test`.
