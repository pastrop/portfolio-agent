...
| **Generator** | Constructs a concrete portfolio allocation that satisfies the spec. Returns plain-English `descriptions` for each ticker alongside `allocations`. Its diversification prompt **budgets by risk factor, not product label**: it maps instruments to the ~7 macro factors (equity beta, rates/duration, inflation, commodities, gold, trend, cash) and caps each — at most one rates sleeve (IG credit / agg are ~90% duration, not independent) and at most one equity tilt beyond core+intl+EM (small-cap/REIT/HY are equity beta). On later iterations, addresses every point from the evaluator's critique. |
| **Evaluator** | Independently stress-tests the portfolio against five 1–10 criteria (constraint compliance, return potential, diversification, implementability, methodology rigour) and writes a detailed critique. **Passes only if** `avg ≥ 7` AND `no single score ≤ 4` AND the evaluator's own `passed: true/false` judgement agrees — all three must hold. Two **deterministic gates** run on top and can only turn a pass into a fail: the horizon **growth-asset ceiling** and the **factor budget** (≥ 2 rates sleeves, or ≥ 2 equity tilts → forced fail). |
...
3. **Iteration feedback is regime-aware.** Each round's feedback is shaped by the Evaluator's result: when an iteration passes but its drawdown is over target (`TARGET_MAX_LOSS`) the Generator is told to push it down; when drawdown is well under target (≤ `UNDER_UTILISATION_BAND`) the Generator is told the risk budget is being wasted; failed iterations pass the critique through verbatim. (An earlier design also prepended per-iteration Advisor correlation pairs here; that was removed when correlation moved to a deterministic post-processing step — see decision 4. Redundant holdings are now headed off at construction time by the Generator's factor-budget prompt and enforced by the Evaluator's factor-budget gate — see the Generator/Evaluator roles above.)
...
- **Deterministic constraint checker.** Add a Python pre-check that verifies hard rules (ticker count ≤ 15, sector caps, leverage, instruments restricted to the spec's `asset_universe`) before the LLM Evaluator sees the portfolio. Cheap, deterministic, no opinion drift. Would also catch the "Generator used a ticker outside the asset_universe" failure mode the Evaluator currently has to spot manually. (Two deterministic gates already exist — the horizon growth-asset ceiling and the **factor budget** in `harness.factor_budget_violations`, which fails a book that stacks redundant rates sleeves or equity tilts. A further step is a computed-ρ gate: `correlation.py` already measures real correlations for *reporting*, so the Evaluator could fail any *unjustified* pair over, say, `|ρ| > 0.85` — catching novel cousins the category-based factor gate doesn't enumerate.)
